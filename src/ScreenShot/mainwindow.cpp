    ...
    ui(new Ui::MainWindow)
{
    ...
    //初始化大小
    int win_width = config["width"].toInt();
    int win_height = config["height"].toInt();
...
{
    int last_index = ui->subject->currentIndex();
    QListWidgetItem * item = ui->subject_list->currentItem();
    if( item == NULL )
        return;
    int curIndex = ui->subject_list->row(item);
    QString old_name = items.at(curIndex).toString();
    //qDebug() << "调用了" << curIndex;
    QString new_name = QInputDialog::getText(this, "输入", "请输入新名称:", QLineEdit::Normal, old_name);
    QString temp_str = new_name;
    temp_str.remove(QRegExp("\\s")); //去除特殊字符
    if (!temp_str.isEmpty())
    {
        if (temp_str == old_name) //没更改还点了确定
            return;
        ...
        for (int i = 0; i < items.count(); i++)
        {
            if (items[i] == temp_str)
            {
                QMessageBox::critical(this, "错误", "存在重名项目");
                return; //有重名，取消
            }
        }
        //重命名文件夹
        QDir dir(working_path);
        if (dir.exists())
        {
            if (!dir.rename(old_name, new_name)) //文件夹重命名失败
            {
                if (!dir.mkdir(new_name))
                {
                    QMessageBox::critical(this, "错误", "文件夹重命名失败,尝试创建文件夹仍然失败");
                    return;
                }
            }
        }
        ...
        {
            QMessageBox::critical(this, "错误", "源工作目录不存在");
            return;
        }
    }
    else
    ...
    //替换数据
    ui->subject_list->takeItem(curIndex);
    ui->subject_list->insertItem(curIndex, new_name);
    items.removeAt(curIndex);
    items.insert(curIndex, new_name);
    ui->subject->removeItem(curIndex);
    ui->subject->insertItem(curIndex, new_name);
    ui->subject->setCurrentIndex(last_index);
...
void MainWindow::AddSubject()
{
    ...
    QString subject_name = QInputDialog::getText(this, "新项目", "请输入项目名称:");
...
void MainWindow::TrayRightClick()
{
    ...
    QAction* current_subject = new QAction(ui->subject->currentText(), this);
    current_subject->setIcon(QIcon(":/pic/subject.png"));
    ...
    connect(current_subject, &QAction::triggered, this, &MainWindow::OpenSubjectPath);
    ...
    menu->addAction(current_subject);
    menu->addSeparator();
